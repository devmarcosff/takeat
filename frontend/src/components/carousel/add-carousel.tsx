      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex [touch-action:pan-y] ml-[calc(var(--slide-spacing)*-1)]">
          ...
          {restaurants?.map((item, index) => {
            if (item?.restaurant?.status) {
              return (
                <Link href={`/${item.restaurant?.phone}`} className={`cursor-pointer select-none transform-gpu translate-x-0 translate-y-0 flex-shrink-0 flex-grow-0 min-w-[var(--slide-size)] md:w-1/6 px-[var(--slide-spacing)]`} key={index}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-start gap-3 w-full justify-between">
                        <div>
                          ...
                          <p className="text-sm flex items-center gap-1">{item.restaurant?.categoria || 'Geral'}</p>
                        </div>
                        <div>
                          ...
                          <span className="flex items-center gap-2 text-xs font-light mt-1">{item.restaurant?.status == 1 ? '08:30 as 22:30' : ''}</span>
                        </div>
                      </div>
                    </div>
                  </div>
